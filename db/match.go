...
func UpdateMatchStatus(matchID string, status string) error {
	return Get().Model(&dao.Match{}).Where("match_id = ?", matchID).Update("status", status).Error
}

// UpdatePlayerStatus 更新指定玩家的确认状态
func UpdatePlayerStatus(matchID string, playerAddress string, status string) error {
	return Get().Model(&dao.Match{}).Where("match_id = ? AND address = ?", matchID, playerAddress).Update("status", status).Error
}
