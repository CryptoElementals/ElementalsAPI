...
case $TEST_TYPE in
    all)
        ...
        go test -v . -run "TestUpdateCardStatByPlayerIDs|TestUpdateUserStatByPlayerIds"
        ;;
    card)
        echo -e "${BLUE}=== 运行卡牌统计测试 ===${NC}"
        go test -v . -run TestUpdateCardStatByPlayerIDs
        ;;
    user)
        echo -e "${BLUE}=== 运行用户统计测试 ===${NC}"
        go test -v . -run TestUpdateUserStatByPlayerIds
        ;;
    card-new)
        echo -e "${BLUE}=== 运行卡牌统计新玩家测试 ===${NC}"
        go test -v . -run TestUpdateCardStatByPlayerIDs_NewPlayer
        ;;
    user-new)
        echo -e "${BLUE}=== 运行用户统计新玩家测试 ===${NC}"
        go test -v . -run TestUpdateUserStatByPlayerIds_NewPlayer
        ;;
    user-multiple)
        echo -e "${BLUE}=== 运行用户统计多玩家测试 ===${NC}"
        go test -v . -run TestUpdateUserStatByPlayerIds_MultiplePlayers
        ;;
    coverage)
        echo -e "${BLUE}=== 运行测试并生成覆盖率报告 ===${NC}"
        go test -v . -run "TestUpdateCardStatByPlayerIDs|TestUpdateUserStatByPlayerIds" -coverprofile=coverage.out
