...
const (
	LobbySettlementService_NotifyGameCompleted_FullMethodName = "/rpc.LobbySettlementService/NotifyGameCompleted"
)

// LobbySettlementServiceClient is the client API for LobbySettlementService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// LobbySettlementService: room notifies lobby when a game completes (queue + tournament settlement).
type LobbySettlementServiceClient interface {
	NotifyGameCompleted(ctx context.Context, in *NotifyGameCompletedRequest, opts ...grpc.CallOption) (*emptypb.Empty, error)
}

type lobbySettlementServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewLobbySettlementServiceClient(cc grpc.ClientConnInterface) LobbySettlementServiceClient {
	return &lobbySettlementServiceClient{cc}
}

func (c *lobbySettlementServiceClient) NotifyGameCompleted(ctx context.Context, in *NotifyGameCompletedRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, LobbySettlementService_NotifyGameCompleted_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// LobbySettlementServiceServer is the server API for LobbySettlementService service.
// All implementations must embed UnimplementedLobbySettlementServiceServer
// for forward compatibility.
//
// LobbySettlementService: room notifies lobby when a game completes (queue + tournament settlement).
type LobbySettlementServiceServer interface {
	NotifyGameCompleted(context.Context, *NotifyGameCompletedRequest) (*emptypb.Empty, error)
	mustEmbedUnimplementedLobbySettlementServiceServer()
}

// UnimplementedLobbySettlementServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedLobbySettlementServiceServer struct{}

func (UnimplementedLobbySettlementServiceServer) NotifyGameCompleted(context.Context, *NotifyGameCompletedRequest) (*emptypb.Empty, error) {
	return nil, status.Errorf(codes.Unimplemented, "method NotifyGameCompleted not implemented")
}
func (UnimplementedLobbySettlementServiceServer) mustEmbedUnimplementedLobbySettlementServiceServer() {
}
func (UnimplementedLobbySettlementServiceServer) testEmbeddedByValue() {}

// UnsafeLobbySettlementServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to LobbySettlementServiceServer will
// result in compilation errors.
type UnsafeLobbySettlementServiceServer interface {
	mustEmbedUnimplementedLobbySettlementServiceServer()
}

func RegisterLobbySettlementServiceServer(s grpc.ServiceRegistrar, srv LobbySettlementServiceServer) {
	// If the following call pancis, it indicates UnimplementedLobbySettlementServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&LobbySettlementService_ServiceDesc, srv)
}

func _LobbySettlementService_NotifyGameCompleted_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(NotifyGameCompletedRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(LobbySettlementServiceServer).NotifyGameCompleted(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: LobbySettlementService_NotifyGameCompleted_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(LobbySettlementServiceServer).NotifyGameCompleted(ctx, req.(*NotifyGameCompletedRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// LobbySettlementService_ServiceDesc is the grpc.ServiceDesc for LobbySettlementService service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var LobbySettlementService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "rpc.LobbySettlementService",
	HandlerType: (*LobbySettlementServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "NotifyGameCompleted",
			Handler:    _LobbySettlementService_NotifyGameCompleted_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "rpc/proto/lobby.proto",
